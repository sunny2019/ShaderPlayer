...
namespace CKit
{
    ...
    public class MeshData
    {
        ...
        //把网格数据保存成文本信息
        private void SaveMeshData(StringBuilder data, MeshFilter meshFilter,
            Dictionary<string, MaterialData> materialDic)
        {
            //保存网格名称
        ...
        //保存材质数据
        private void SaveMaterails(MeshFilter meshFilter, StringBuilder data,
            Dictionary<string, MaterialData> materialDic)
        {
                ...
                if (!materialDic.ContainsKey(materialName))
                {
                    ...
                    materialData.TextureName =
                        materialArray[materialIndex].mainTexture
                            ? AssetDatabase.GetAssetPath(materialArray[materialIndex].mainTexture)
                            : null;
