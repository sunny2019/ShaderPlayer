...
namespace CKit
{
    ...
    {
        [MenuItem("CKit/导出模型/将选中模型分别导出（子物体会拆分导出）")]
        private static void ExportAllChild()
        {
            ...
            for (int i = 0; i < selection.Length; i++)
            {
                ...
                {
                    exportCount++;
                    ExportFile.ExportObjToOne((MeshFilter) meshfilter[m], ExportFile.EXPORT_FOLDER,
                        selection[i].name + "_" + i + "_" + m);
                }
            }
        ...
        [MenuItem("CKit/导出模型/将选中模型导出成一个obj文件")]
        ...
        [MenuItem("CKit/导出模型/将选中模型分别导出（子物体不拆分导出）")]
